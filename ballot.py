...
def tex(config_file: io.BufferedReader):
    ...
    for name, paper in config["papers"].items():
        ...
        generate_ballot_tex(name, config["config"]["org"], paper["title"], paper["date"], candidates)
...
def texpack(config_file: io.BufferedReader):
    ...
    render = template.render(
        ballot_list=config["papers"].items()
    )
...
def generate_ballot_tex(name: str, org: str, title: str, date: date, candidates: List[str]):
